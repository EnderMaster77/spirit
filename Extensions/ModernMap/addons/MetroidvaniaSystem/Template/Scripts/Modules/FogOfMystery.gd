...
func _initialize():
	...
	var grad := preload(
		"res://addons/MetroidvaniaSystem/Template/Scripts/Modules/FogDiscoverGradient.tres"
	)
	...
	compositor_base.material = preload(
		"res://addons/MetroidvaniaSystem/Template/Scripts/Modules/FogMaterial.tres"
	)
...
func on_new_room():
	...
	discover_render.size = (
		MetSys.CELL_SIZE * (Vector2(room.get_end_coords() - room.get_base_coords()) + Vector2(1, 1))
	)
...
func cache_overlays_from_discovers(overlays_to_cache: PackedStringArray):
	...
	for room in overlays_to_cache:
		...
		overlay_cache[room] = ImageTexture.create_from_image(
			sub_compositor.get_texture().get_image()
		)
...
static func draw_overlay(canvas_item: CanvasItem, coords: Vector3i, pos: Vector2, size: Vector2):
	...
	if Engine.is_editor_hint() and not fom:
		fom = load("res://addons/MetroidvaniaSystem/Template/Scripts/Modules/FogOfMystery.gd").new(
			null
		)
