...
const FogOfMystery = preload(
	"res://addons/MetroidvaniaSystem/Template/Scripts/Modules/FogOfMystery.gd"
)
...
func draw_elevator(
	canvas_item: CanvasItem, coords: Vector3i, pos: Vector2, size: Vector2, data: String
):
	# For elevator to display, both top and bottom cell must be discovered.
	if (
		not MetSys.is_cell_discovered(coords + Vector3i(0, -1, 0))
		and not MetSys.is_cell_discovered(coords + Vector3i(0, size.y, 0))
	):
		return
...
func draw_label(
	...
	canvas_item.draw_string(
		...
		pos + Vector2(-1, -0.5) * MetSys.CELL_SIZE,
		data,
		HORIZONTAL_ALIGNMENT_CENTER,
		MetSys.CELL_SIZE.x * 3
	)
