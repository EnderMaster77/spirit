...
func _run() -> void:
	...
	for y in size.y:
		for x in size.x:
			...
			if not check_cell(Vector2i(x, y), tmcells):
				var v: float
				if (
					check_cell(Vector2i(x + 1, y), tmcells)
					or check_cell(Vector2i(x - 1, y), tmcells)
					or check_cell(Vector2i(x, y + 1), tmcells)
					or check_cell(Vector2i(x, y - 1), tmcells)
				):
					v = 1.0
	...
	image.save_png(
		(
			(
				preload("res://addons/MetroidvaniaSystem/Template/Scripts/Modules/FogOfMystery.gd")
				. OVERLAYS_PATH
				. path_join(room_name)
			)
			+ ".png"
