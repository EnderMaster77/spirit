...
func _input(event: InputEvent) -> void:
	if Input.is_action_just_released("switchElement"):
		switch_to_element.emit(get_closest_to_mouse($CanvasLayer.get_children()).element_to_switch_to)
		disable()
...
func get_closest_to_mouse(nodes:Array): # Nodes is array with node2Ds.
	...
	for node in nodes:
		if typeof(node) != 24:
			continue
		var distance: float = node.position.distance_to($CanvasLayer.get_viewport().get_mouse_position())
