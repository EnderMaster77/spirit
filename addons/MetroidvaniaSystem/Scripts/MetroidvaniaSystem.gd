...
const CustomElementManager = preload(
	"res://addons/MetroidvaniaSystem/Scripts/CustomElementManager.gd"
)
...
func _enter_tree() -> void:
	...
	if ProjectSettings.has_setting("addons/metroidvania_system/settings_file"):
		...
		var legacy_settings_path: String = ProjectSettings.get_setting(
			"metroidvania_system/settings_file"
		)
		if legacy_settings_path != settings_path:
			push_warning(
				'Detected MetSys settings path under "metroidvania_system/settings_file" project setting. Migrating it to the new setting: "addons/metroidvania_system/settings_file".'
			)
	...
	ProjectSettings.add_property_info(
		{
			"name": "addons/metroidvania_system/settings_file",
			"type": TYPE_STRING,
...
func visit_cell(coords: Vector3i):
	...
	var previous_map := map_data.get_assigned_scene_at(
		Vector3i(last_player_position.x, last_player_position.y, current_layer)
	)
...
func set_player_position(position: Vector2):
	...
	var player_pos := (
		Vector2i((position / settings.in_game_cell_size).floor()) + current_room.min_cell
	)
...
func register_storable_object_with_marker(
	object: Object, stored_callback := Callable(), map_marker := DEFAULT_SYMBOL
) -> bool:
...
func get_object_id(object: Object) -> String:
	if object.has_meta(&"object_id"):
		...
		var id := str(
			object.owner.scene_file_path.get_file().get_basename(),
			"/",
...
func get_cell_position(
	coords: Vector2i, relative := Vector2(0.5, 0.5), base_offset := Vector2()
) -> Vector2:
...
func draw_cell(
	...
	RoomDrawer.draw(
		canvas_item, offset, coords, skip_empty, map_data, save_data if use_save_data else null
	)
...
func draw_custom_elements(
	canvas_item: CanvasItem, rect: Rect2i, drawing_offset := Vector2(), layer := current_layer
):
	if not settings.custom_elements or map_data.custom_elements.is_empty():
		return
	RoomDrawer.draw_custom_elements(
		canvas_item, map_data.custom_elements, drawing_offset, rect, layer
	)
...
func add_player_location(canvas_item: CanvasItem, offset := Vector2()) -> Node2D:
	...
	location_instance.set_script(
		load("res://addons/MetroidvaniaSystem/Scripts/PlayerLocationInstance.gd")
	)
