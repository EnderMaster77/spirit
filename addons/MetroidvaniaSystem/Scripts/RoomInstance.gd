	...
	for coords in cells:
		...
		for i in 4:
			...
			if cell_data.borders[i] > 0:
				...
				var preview: Control = (
					load("res://addons/MetroidvaniaSystem/Nodes/RoomPreview.tscn").instantiate()
				)
				...
				preview.add_room(
					temp_map, i, next_min_cell - Vector2i(next_coords.x, next_coords.y)
				)
...
func get_local_cells() -> Array[Vector2i]:
	...
	ret.assign(
		cells.map(
			func(coords: Vector3i) -> Vector2i: return Vector2i(
				coords.x - min_cell.x, coords.y - min_cell.y
			)
		)
	)
	return ret
...
func get_neighbor_rooms() -> Array[String]:
	...
	for coords in cells:
		...
		for i in 4:
			...
			if cell_data.borders[i] > 0:
				var scene: String = MetSys.map_data.get_assigned_scene_at(
					coords + Vector3i(fwd.x, fwd.y, 0)
				)
			...
			match i:
				MetroidvaniaSystem.R:
					draw_rect(
						Rect2(
							(
								(coords + Vector2.RIGHT) * MetSys.settings.in_game_cell_size
								+ Vector2(-width, 0)
							),
							Vector2(width, MetSys.settings.in_game_cell_size.y)
						),
						color
					)
				MetroidvaniaSystem.D:
					draw_rect(
						Rect2(
							(
								(coords + Vector2.DOWN) * MetSys.settings.in_game_cell_size
								+ Vector2(0, -width)
							),
							Vector2(MetSys.settings.in_game_cell_size.x, width)
						),
						color
					)
				MetroidvaniaSystem.L:
					draw_rect(
						Rect2(
							coords * MetSys.settings.in_game_cell_size,
							Vector2(width, MetSys.settings.in_game_cell_size.y)
						),
						color
					)
