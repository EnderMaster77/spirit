const FWD = {
	MetroidvaniaSystem.R: Vector2i.RIGHT,
	MetroidvaniaSystem.D: Vector2i.DOWN,
	MetroidvaniaSystem.L: Vector2i.LEFT,
	MetroidvaniaSystem.U: Vector2i.UP
}
	...
	func get_string() -> String:
		...
		if colors.any(func(col: Color): return col.a > 0):
			data.append(
				(
					"%s,%s,%s,%s,%s"
					% colors.map(func(col: Color): return col.to_html(false) if col.a > 0 else "")
				)
			)
		else:
			data.append("")
...
class CellOverride:
	...
	static func load_from_line(line: String) -> CellOverride:
		...
		var coords := Vector3i(
			coords_string.get_slice(",", 0).to_int(),
			coords_string.get_slice(",", 1).to_int(),
			coords_string.get_slice(",", 2).to_int()
		)
...
func load_data():
	var file := FileAccess.open(
		MetSys.settings.map_root_folder.path_join("MapData.txt"), FileAccess.READ
	)
	if not file:
	...
	while i < data.size():
		...
		if line.begins_with("$ln"):
			...
			element.size = Vector2i(
				element_data[2].get_slice("x", 0).to_int(),
				element_data[2].get_slice("x", 1).to_int()
			)
...
func save_data():
	var file := FileAccess.open(
		MetSys.settings.map_root_folder.path_join("MapData.txt"), FileAccess.WRITE
	)
	if not file:
		push_error(
			(
				"Could not open file '%s' for writing."
				% MetSys.settings.map_root_folder.path_join("MapData.txt")
			)
		)
		return
...
func get_room_from_scene_path(path: String, safe := true) -> String:
	...
	if not room_name in assigned_scenes and not room_name in scene_overrides:
		room_name = ResourceUID.id_to_text(ResourceLoader.get_resource_uid(path)).replace(
			"uid://", ":"
		)
