...
static func draw(
	...
	coords: Vector3i,
	skip_empty: bool,
	map_data: MetroidvaniaSystem.MapData,
	save_data: MetroidvaniaSystem.MetSysSaveData
):
	...
	if bool(display_flags & MetroidvaniaSystem.DISPLAY_CENTER):
		...
		theme.center_texture.draw(
			ci,
			offset * MetSys.CELL_SIZE,
			room_color if discovered == 2 else theme.mapped_center_color
		)
	...
	if theme.use_shared_borders:
		if not MetSys.has_meta(&"shared_borders_to_draw"):
			...
			MetSys._add_meta(
				&"shared_borders_data", {"canvas_item": canvas_item, "display_flags": display_flags}
			)

		var shared_borders_to_draw: Dictionary = MetSys.get_meta(&"shared_borders_to_draw")
		shared_borders_to_draw[coords] = offset
	else:
		draw_regular_borders(
			canvas_item, offset, coords, map_data, cell_data, display_flags, discovered
		)
	...
	if bool(display_flags & MetroidvaniaSystem.DISPLAY_SYMBOLS):
		...
		if symbol > -1:
			if symbol >= theme.symbols.size():
				...
				canvas_item.draw_texture(
					theme.symbols[symbol],
					(
						offset * MetSys.CELL_SIZE
						+ MetSys.CELL_SIZE * 0.5
						- theme.symbols[symbol].get_size() / 2
					)
				)


static func draw_regular_borders(
	canvas_item: CanvasItem,
	offset: Vector2,
	coords: Vector3i,
	map_data: MetroidvaniaSystem.MapData,
	cell_data: MetroidvaniaSystem.MapData.CellData,
	display_flags: int,
	discovered: int
):
	var theme: MapTheme = MetSys.settings.theme
	...
	if display_outlines and get_border_texture(theme, -1, 0):
		var separators := 2
		if (
			theme.separator_mode == MapTheme.SeparatorMode.EMPTY_DOUBLE
			or theme.separator_mode == MapTheme.SeparatorMode.FULL_DOUBLE
		):
			separators = 4

		var full := (
			theme.separator_mode == MapTheme.SeparatorMode.FULL_DOUBLE
			or theme.separator_mode == MapTheme.SeparatorMode.FULL_SINGLE
		)
	...
	for i in 4:
		...
		if discovered == 2:
			corner_color = get_shared_color(
				cell_data.get_border_color(i),
				cell_data.get_border_color(j),
		...
		canvas_item.draw_set_transform(
			offset * MetSys.CELL_SIZE + MetSys.CELL_SIZE * 0.5, PI * 0.5 * i, Vector2.ONE
		)
		...
		match i:
			...
			MetroidvaniaSystem.D, MetroidvaniaSystem.U:
				texture.draw(
					ci,
					Vector2(MetSys.CELL_SIZE.y, MetSys.CELL_SIZE.x) * 0.5 + corner_offset,
					corner_color
				)
	...
	for i in 4:
		...
		if neighbor_room:
			if (
				neighbor_room.borders[opposite(i)] == -1
				and neighbor_room.borders[opposite(j)] == -1
			):
				continue

		var texture: Texture2D = theme.inner_corner
		var color1 := get_neighbor(map_data, coords, map_data.FWD[i]).get_border_color(rotate(i))
		var color2 := get_neighbor(map_data, coords, map_data.FWD[j]).get_border_color(
			rotate(j, -1)
		)
...
static func draw_shared_borders():
	assert(
		MetSys.settings.theme.use_shared_borders,
		"This function requires shared borders to be enabled."
	)
	if not MetSys.has_meta(&"shared_borders_to_draw"):
		return
	...
	for coords in shared_borders_to_draw:
		...
		corner_data |= (
			(1 << 0)
			* (
				get_corner_bit(coords, MetroidvaniaSystem.R)
				| get_corner_bit(coords + Vector3i(1, 0, 0), MetroidvaniaSystem.L)
			)
		)
		corner_data |= (
			(1 << 1)
			* (
				get_corner_bit(coords, MetroidvaniaSystem.D)
				| get_corner_bit(coords + Vector3i(0, 1, 0), MetroidvaniaSystem.U)
			)
		)
		corner_data |= (
			(1 << 2)
			* (
				get_corner_bit(coords + Vector3i(0, 1, 0), MetroidvaniaSystem.R)
				| get_corner_bit(coords + Vector3i(1, 1, 0), MetroidvaniaSystem.L)
			)
		)
		corner_data |= (
			(1 << 3)
			* (
				get_corner_bit(coords + Vector3i(1, 0, 0), MetroidvaniaSystem.D)
				| get_corner_bit(coords + Vector3i(1, 1, 0), MetroidvaniaSystem.U)
			)
		)
		...
		if not color_blend.is_empty():
			...
			shared_corners_colors[coords] = color_blend.reduce(
				func(final: Color, current: Color) -> Color: return final.lerp(current, ratio)
			)
		elif use_default:
		...
		for i in 2:
			var fwd := Vector3i(
				MetroidvaniaSystem.MapData.FWD[i].x, MetroidvaniaSystem.MapData.FWD[i].y, 0
			)
			border_data[i] = maxi(
				get_border_at(coords, i), get_border_at(coords + fwd, opposite(i))
			)
	...
	for coords in shared_borders_to_draw:
		...
		for i in 2:
			...
			canvas_item.draw_set_transform(
				shared_borders_to_draw[coords] * MetSys.CELL_SIZE + MetSys.CELL_SIZE * 0.5,
				rotation,
				Vector2.ONE
			)
			match i:
				MetroidvaniaSystem.R, MetroidvaniaSystem.L:
					texture.draw(
						ci,
						-texture.get_size() / 2 + Vector2.RIGHT * (MetSys.CELL_SIZE.x * 0.5),
						color
					)
				MetroidvaniaSystem.D, MetroidvaniaSystem.U:
					texture.draw(
						ci,
						-texture.get_size() / 2 + Vector2.RIGHT * (MetSys.CELL_SIZE.y * 0.5),
						color
					)
	...
	for coords in shared_borders_to_draw:
		...
		canvas_item.draw_set_transform(
			offset * MetSys.CELL_SIZE + MetSys.CELL_SIZE, PI * 0.5 * corner_rotation, Vector2.ONE
		)
		corner_texture.draw(
			canvas_item.get_canvas_item(),
			-corner_texture.get_size() / 2,
			shared_corners_colors[coords]
		)
...
static func draw_custom_elements(
	canvas_item: CanvasItem, elements: Dictionary, base_offset: Vector2, rect: Rect2i, layer: int
):
	...
	for coords in elements:
		...
		if element_rect.intersects(rect):
			element_manager.draw_element(
				canvas_item,
				coords,
				element["name"],
				(
					(base_offset + Vector2(coords.x, coords.y) - Vector2(rect.position))
					* MetSys.CELL_SIZE
				),
				Vector2(element_rect.size) * MetSys.CELL_SIZE,
				element.data
			)
...
static func get_shared_border_color(coords: Vector3i, idx: int) -> Color:
	var fwd := Vector3i(
		MetroidvaniaSystem.MapData.FWD[idx].x, MetroidvaniaSystem.MapData.FWD[idx].y, 0
	)
	...
	if cell_data:
		if color.a > 0:
			color = get_shared_color(
				color,
				cell_data.get_border_color(opposite(idx)),
				MetSys.settings.theme.default_border_color
			)
		else:
...
static func draw_border(
	canvas_item: CanvasItem, offset: Vector2, i: int, texture: Texture2D, color: Color
):
	var rotation := PI * 0.5 * i
	canvas_item.draw_set_transform(
		offset * MetSys.CELL_SIZE + MetSys.CELL_SIZE * 0.5, rotation, Vector2.ONE
	)
	match i:
		MetroidvaniaSystem.R, MetroidvaniaSystem.L:
			texture.draw(
				...
				(
					-texture.get_size() / 2
					+ Vector2.RIGHT * (MetSys.CELL_SIZE.x * 0.5 - texture.get_width() / 2)
				),
				color
			)
		MetroidvaniaSystem.D, MetroidvaniaSystem.U:
			texture.draw(
				...
				(
					-texture.get_size() / 2
					+ Vector2.RIGHT * (MetSys.CELL_SIZE.y * 0.5 - texture.get_width() / 2)
				),
				color
			)
...
static func get_neighbor(
	map_data: MetroidvaniaSystem.MapData, coords: Vector3i, offset: Vector2i
) -> MetroidvaniaSystem.MapData.CellData:
