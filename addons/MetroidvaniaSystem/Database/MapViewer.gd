...
func _notification(what: int) -> void:
	if what == NOTIFICATION_THEME_CHANGED:
		...
		theme_cache.foreign_marked_collectible_room = get_theme_color(
			&"foreign_marked_collectible_room", &"MetSys"
		)
...
func _update_status_label():
	...
	if room_under_cursor and not room_under_cursor.assigned_scene.is_empty():
		status_label.text = str(
			get_cursor_pos(), " ", get_assigned_scene_display(room_under_cursor.assigned_scene)
		)
	else:
...
func _on_overlay_input(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_LEFT:
			if (
				event.pressed
				and room_under_cursor
				and not room_under_cursor.assigned_scene.is_empty()
			):
				EditorInterface.open_scene_from_path(
					MetSys.settings.map_root_folder.path_join(
						MetSys.map_data.get_uid_room(room_under_cursor.assigned_scene)
					)
				)
...
func _on_overlay_draw() -> void:
	...
	if cursor_inside:
		map_overlay.draw_rect(
			Rect2(Vector2(mouse + map_offset) * MetSys.CELL_SIZE, MetSys.CELL_SIZE),
			theme_cache.cursor_color,
			false,
			2
		)
	...
	if (
		...
		and get_tree().edited_scene_root.scene_file_path.begins_with(
		...
		for coords in MetSys.map_data.get_cells_assigned_to_path(current_scene):
			...
			map_overlay.draw_rect(
				Rect2(
					Vector2(coords.x + map_offset.x, coords.y + map_offset.y) * MetSys.CELL_SIZE,
					MetSys.CELL_SIZE
				),
				theme_cache.current_scene_room
			)
	...
	if current_hovered_item:
		...
		if "coords" in data:
			...
			map_overlay.draw_rect(
				...
				(
					theme_cache.marked_collectible_room
					if coords.z == current_layer
					else theme_cache.foreign_marked_collectible_room
				)
			)
		else:
			for coords in MetSys.map_data.get_cells_assigned_to(data.map):
				...
				map_overlay.draw_rect(
					Rect2(
						(
							Vector2(coords.x + map_offset.x, coords.y + map_offset.y)
							* MetSys.CELL_SIZE
						),
						MetSys.CELL_SIZE
					),
