...
func _get_configuration_warnings() -> PackedStringArray:
	...
	if smooth_scroll and not MetSys.settings.theme.show_exact_player_location:
		(
			ret
			. append(
				"Smooth Scroll is enabled, but the MapTheme doesn't show exact player location. The property will have no effect."
			)
		)
...
func update_drawer_position():
	var new_position := (
		(
			-(MetSys.exact_player_position / MetSys.settings.in_game_cell_size).posmod(1)
			* MetSys.CELL_SIZE
		)
...
func _draw_map() -> void:
	...
	if player_location:
		...
		if player_location.visible:
			player_location.offset = (
				(-Vector2(draw_center + offset) + draw_offset) * MetSys.CELL_SIZE
			)
	...
	for y in draw_area.y:
		for x in draw_area.x:
			MetSys.draw_cell(
				drawer,
				Vector2(x, y) + draw_offset,
				Vector3i(draw_center.x + offset.x + x, draw_center.y + offset.y + y, layer)
			)
