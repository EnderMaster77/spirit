...
func _forward_canvas_gui_input(event: InputEvent) -> bool:
	if event is InputEventMouseMotion:
		...
		if hovered_preview and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
			var zoom := (
				EditorInterface.get_editor_viewport_2d().global_canvas_transform.get_scale().x
			)
			var pos: Vector2

			var editor = EditorInterface.get_base_control().find_child(
				"@CanvasItemEditorViewport*", true, false
			)
			...
			EditorInterface.open_scene_from_path(
				MetSys.get_full_room_path(hovered_preview.tooltip_text)
			)
			await get_tree().process_frame
